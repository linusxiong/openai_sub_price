...
function RefreshButton({ isRefreshing, onRefresh }: HeaderProps) {
...
export function Header({ isRefreshing, onRefresh }: HeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-20 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50/90 dark:bg-zinc-950/90 backdrop-blur-md">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-2 sm:py-0 min-h-14 sm:h-14 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-4">
        ...
        <div className="flex items-center gap-1.5 w-full sm:w-auto min-w-0">
          ...
          <LanguageSelector />
          <CurrencySelector />
          <div className="hidden sm:block w-px h-4 bg-zinc-200 dark:bg-zinc-700 mx-0.5" />
          <div className="hidden sm:block">
            <ThemeToggle />
          </div>
        </div>
